  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-2">
        {previews.map((url) => (
          <div key={url} className="relative w-20 h-20 overflow-hidden bg-depth">
        ...
        {previews.length < 5 && (
          <Button
            ...
            variant="outline"
            size="icon"
            className="w-20 h-20 border-ghost hover:border-gold"
            onClick={() => fileInputRef.current?.click()}
            disabled={uploading}
          >
            <Upload className="h-6 w-6 text-dim" />
          </Button>
        )}
      </div>
      {uploading && <p className="text-xs text-dim">Uploading...</p>}
    </div>
  );
}
